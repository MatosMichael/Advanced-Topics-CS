...
walls = [wall for sublist in _walls for wall in sublist if wall["cells"][0] >= 0 and wall["cells"][1] >= 0 and wall["cells"][0] < num_rooms and wall["cells"][1] < num_rooms]
swap = [1, 0]
def add_adjacent(i):
    for wall in walls:
        if i in wall["cells"]:
...
player = pygame.Rect((room_size - player_size) // 2, (room_size - player_size) // 2, player_size, player_size)
goal = pygame.Rect((width - room_size) + ((room_size - goal_size) // 2), (height - room_size) + ((room_size - goal_size) // 2), room_size, room_size)
wall_rects = []
game_over = False
while True:
    screen.fill("black")
    ...
    if len(adjacent) > 0:
        ...
        walls = [wall for wall in walls if prox not in wall["cells"] or current not in wall["cells"]]
        maze.add(current)
        ...
        if player.colliderect(goal):
            game_over = True
    wall_rects = [pygame.draw.line(screen, (255,255,255), wall["points"][0], wall["points"][1]) for wall in walls]
